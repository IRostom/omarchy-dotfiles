...
BarWidget {
  ...
  // Snap instead of animate: the other pill (center/Indicators) reveals
  // instantly, so this stays consistent with it instead of running its
  // own 600ms slide.
  ...
  Component {
    ...
    Item {
      ...
      readonly property int chevronWidth: root.allItems.length > 0 ? expandIcon.implicitWidth : 0
      // Reveal width: 0 when collapsed, drawerExtent when fully expanded.
      // Same quantity drives both the actual layout below and the width we
      // report upward, so a chip/pill wrapping this widget always hugs
      // exactly what's painted instead of a stale full-drawer reserve.
      readonly property int revealWidth: root.revealExtent
      readonly property int drawerAreaWidth: chevronWidth + revealWidth

      implicitWidth: drawerAreaWidth + pinnedWidth
      implicitHeight: root.barSize

      // Hit-test only the currently-visible area (chevron + whatever's
      // revealed so far), not a pre-reserved zone past it.
      ...
      Item {
        id: drawerArea
        x: 0
        width: horizontalTrayRoot.drawerAreaWidth
        height: root.barSize
        ...
        Item {
          ...
          width: horizontalTrayRoot.revealWidth
          height: root.barSize
          clip: true
