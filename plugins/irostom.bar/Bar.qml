...
Item {
  ...
  // macOS-style frosted glass: chips stay translucent instead of fully
  // opaque so the Hyprland layer blur behind the bar (see the
  // "omarchy-bar" layer_rule in hypr/looknfeel.lua) shows through.
  readonly property real frostedAlpha: 0.55

  function frostedBackground(c) {
    return Qt.rgba(c.r, c.g, c.b, root.frostedAlpha)
  }
  ...
  component BarPanel: PanelWindow {
    ...
    // Chip reskin: back to the full-width bar. Each section (left/center/
    // right) draws its own rounded chip behind it instead of one bar-wide
    // or one bar-spanning pill.
    anchors {
    ...
    readonly property int pillVPad: Style.space(4)
    ...
    Component {
      ...
      Item {
        ...
        Rectangle {
          ...
          anchors.verticalCenter: parent.verticalCenter
          width: leftModules.width > 0 ? leftModules.width + barWindow.chipHPad * 2 : 0
          height: parent.height
          radius: height / 2
          visible: leftModules.width > 0
          color: root.transparent ? "transparent" : root.frostedBackground(root.background)
        }
        LeftModules { id: leftModules; anchors.centerIn: leftChip }
        ...
        Rectangle {
          ...
          anchors.verticalCenter: parent.verticalCenter
          width: rightModules.width > 0 ? rightModules.width + barWindow.chipHPad * 2 : 0
          height: parent.height
          radius: height / 2
          visible: rightModules.width > 0
          color: root.transparent ? "transparent" : root.frostedBackground(root.background)
        }
  ...
  component CenterModules: Item {
    ...
    // Pill-chip reskin: expose the loaded content's implicit size so a
    // caller can size a background chip around this section instead of
    // always being told to fill an already-known parent width.
    ...
    Component {
      ...
      Item {
        ...
        implicitWidth: centerRoot.hasAnchor
          ? beforeList.width + centerAnchorModule.width + afterList.width
          : plainList.width
        implicitHeight: centerRoot.hasAnchor ? centerAnchorModule.height : plainList.height
